...
def load_conversations():
  with open('ml/dialogues.txt', 'r') as file:
  ...
  while i < len(lines) - 1:
    ...
    while i < len(lines) - 1 and lines[i+1] != '\n':
      inputs.append(preprocess_sentence(lines[i][2:-1]))
      outputs.append(preprocess_sentence(lines[i+1][2:-1]))
      i += 1
    i += 1
...
def csv_reader(file_obj):
    ...
    for row in reader:
        if flag == 0:
          ...
          continue
        if(row[0] != ''):
            if(row[1] != ''): i.append(row[1])
            if(row[2] != ''): o.append(row[2])
        else:
        	for first in i:
        		for second in o:
        			inp.append(first)
...
with open("ml/Book.csv", "r") as f:
    i, o = csv_reader(f)

questions, answers = load_conversations()
print(len(i), len(o), len(questions), len(answers))
questions.extend(i)
answers.extend(o)
print(len(i), len(o), len(questions), len(answers))
...
#output = predict("я завтра совершу суецид")

# feed the model with its previous output
'''
sentence = 'я попала на допсу и теперь я все сосу.'
for _ in range(5):
